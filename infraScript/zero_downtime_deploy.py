...
class ServiceManager:
    ...
    def _find_current_service(self) -> None:
        ...
        if not current_service:
            self.current_name, self.current_port = 'pawpatrol_2', self.services['pawpatrol_2']
        else:
    ...
    def _run_container(self, name: str, port: int) -> None:
        os.system(
            f"docker run -d --name={name} --restart unless-stopped -p {port}:8090 "
            f"-e TZ=Asia/Seoul "
            f"-e FIREBASE_CONFIG_PATH=/firebase-config.json "  # 환경 변수 추가
            f"-v /dockerProjects/pawpatrol/volumes/gen:/gen "
            f"-v /dockerProjects/pawpatrol/firebase-config.json:/firebase-config.json "  # Firebase 설정 파일 마운트
            f"--pull always ghcr.io/backendschoolplus3th/pawpatrol")
